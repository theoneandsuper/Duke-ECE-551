...
class Polynomial {
	...
	public:
		...
		numT findZero(numT x, unsigned max_step, const ToleranceT & tolerance, const ToleranceT & deriv_tolerance) const;
};
...
template <typename numT>
numT Polynomial<numT>::eval(const numT & x) const {
	numT result = numT();
	numT x_power = numT(1);

	for (std::size_t i = 0; i < coefficient.size(); i++) {
		result = result + coefficient[i] * x_power;
		x_power = x_power * x;
	}
...
template <typename numT>
Polynomial<numT> Polynomial<numT>::derivative() const {
	Polynomial<numT> answer;
	...
	for (std::size_t i = 1; i < coefficient.size(); i++) {
		if (coefficient[i] == numT()) {
			continue;
		}

		answer.addTerm(numT(i) * coefficient[i], static_cast<unsigned>(i - 1));
	}
	return answer;
}
...
class convergence_failure : public std::exception {
	public:
		...
		explicit convergence_failure(const numT & v) : value(v) {}
...
template <typename numT>
template <typename OtherNumT>
Polynomial<OtherNumT> Polynomial<numT>::convert() const {
	Polynomial<OtherNumT> result;

	for (std::size_t i = 0; i < coefficient.size(); i++) {
		result.addTerm(OtherNumT(coefficient[i]), static_cast<unsigned>(i));
	}
	return result;
}
...
template <typename numT>
template <typename ToleranceT>
numT Polynomial<numT>::findZero(numT x, unsigned max_step, const ToleranceT & tolerance, const ToleranceT & deriv_tolerance) const {
	...
	while (true) {
		...
		numT f_d_x = deriv.eval(x);

		if (std::abs(f_d_x) <= deriv_tolerance) {
		       throw convergence_failure<numT>(x);
	      	}
